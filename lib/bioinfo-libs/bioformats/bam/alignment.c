...
alignment_t* alignment_new_by_bam_small(bam1_t* bam_p, int base_quality) {
    ...
    alignment_p->data_length = 96 + bam_p->l_aux + 3 * bam_p->core.l_qseq + 4 * bam_p->core.n_cigar;
    alignment_p->length = bam_p->core.l_qseq;
    alignment_p->compressed_length = (alignment_p->length * 0.5) + (alignment_p->length % 2);

    alignment_p->sequence = (char*) calloc(alignment_p->length + 1, sizeof(char));
    memcpy(alignment_p->sequence, bam1_seq(bam_p), alignment_p->compressed_length);
    alignment_p->sequence[alignment_p->compressed_length + 1] = '\0';

    // Parse the optional tags
    alignment_p->optional_fields = NULL;
    ...
    if (bam_p->l_aux > 0) {
        ...
        if (alignment_p->optional_tags) {
            // Update the alignment data size
            size_t tag_count = array_list_size(alignment_p->optional_tags);

            for (size_t i = 0; i < tag_count; ++i) {
                bam_tag_t* tg = array_list_get(i, alignment_p->optional_tags);

                switch (tg->type) {
                    case BAM_TAG_TYPE_STRING:
                        {
                            bam_string_t* st = tg->data;
                            alignment_p->data_length += sizeof(bam_tag_t) + st->count;
                        }
                        break;
                    default:
                        alignment_p->data_length += sizeof(bam_tag_t) + sizeof(bam_int_t);
                        break;
                }
            }

            alignment_p->data_length += sizeof(array_list_t);
        }
    }
...
alignment_t* alignment_new_by_bam(bam1_t* bam_p, int base_quality) {
    ...
    convert_to_sequence_string(alignment_p->sequence, bam1_seq(bam_p), bam_p->core.l_qseq);
...
void convert_to_sequence_string(char* decompressed_p, uint8_t* sequence_p, int sequence_length) {
  ...
  decompressed_p[sequence_length] = '\0';
}
...
char* convert_to_quality_string_length(char* quality_dest, uint8_t* quality_src, int quality_length, int base_quality) {
  ...
  quality_dest[quality_length - 1] = '\0';
  return quality_dest;
}
...
alignment_batch_t* alignment_batch_new(size_t batch_size, uint8_t type, uint8_t chromosome) {
  alignment_batch_t* ptr = calloc(1, sizeof(alignment_batch_t));

  ptr->chromosome = chromosome;
  ptr->type = type;
  ptr->alignments = static_queue_new(batch_size);
...
void alignment_batch_free(alignment_batch_t* batch) {
  if (batch) {
    if (batch->alignments) {
      static_queue_free(batch->alignments);
    }
